...
def decode_rotation(clk):
    ...
    if (CLK == 1) and (DT == 0):
        ...
        list_index -= 1
        if list_index < 0 or list_index > len(itemdict):
            list_index = len(itemdict) - 1
        cls()
        print_item(itemdict)
        while CLK == 1:
            CLK = GPIO.input(clk)
...
def button_callback(sw):
    #itemlist.clear()
    print('button pressed at position: ' + str(itemdict[list_index]))
